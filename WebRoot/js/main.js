...
function GetRevAllInfo() {
	...
	for ( var i = 1; i <= vCount; i++) {
		...
		alert(dateObj.getYear() + "年" + dateObj.getMonth() + 1 + "月"
				+ dateObj.getDate() + "日" + dateObj.getHours() + "时"
				+ dateObj.getMinutes() + "分" + dateObj.getSeconds() + "秒");
...
function hideSaveItem() {
	try {
		var webObj = document.getElementById("WebOffice1");
		webObj.SetToolBarButton2("Standard", 3, 0);
	} catch (e) {
		alert("异常\r\nError:" + e + "\r\nError Code:" + e.number
				+ "\r\nError Des:" + e.description);
	}
...
function showSaveItem() {
	try {
		var webObj = document.getElementById("WebOffice1");
		webObj.SetToolBarButton2("Standard", 3, 4);
	} catch (e) {
		alert("异常\r\nError:" + e + "\r\nError Code:" + e.number
				+ "\r\nError Des:" + e.description);
	}
...
function linkRed() {
	window
			.open(
					"mark.html",
					"newwindow",
					'height=768, width=1024, top=0, left=0, toolbar=yes,resizable=yes, menubar=yes,location=yes, status=yes');
}
/*******************************************************************************
 * 
 * 上传文档 /
...
/*******************************************************************************
 * 
 * 设置weboffice自带工具栏“新建文档”隐藏 /
 ******************************************************************************/
function bToolBar_HideNew() {
	try {
		var webObj = document.getElementById("WebOffice1");
		var vCurItem = document.all.WebOffice1.HideMenuItem(0);
		// 根据vCurItem判断当前按钮是否显示
		webObj.HideMenuItem(0x01); // Hide it
	} catch (e) {
		alert("异常\r\nError:" + e + "\r\nError Code:" + e.number
				+ "\r\nError Des:" + e.description);
	}
...
/*******************************************************************************
 * 
 * 设置weboffice自带工具栏“打开文档”隐藏 /
 ******************************************************************************/
function bToolBar_HideOpen() {
	try {
		var webObj = document.getElementById("WebOffice1");
		var vCurItem = webObj.HideMenuItem(0);
		// 根据vCurItem判断当前按钮是否显示
		webObj.HideMenuItem(0x02); // hide it
	} catch (e) {
		alert("异常\r\nError:" + e + "\r\nError Code:" + e.number
				+ "\r\nError Des:" + e.description);
	}
...
/*******************************************************************************
 * 
 * 设置weboffice自带工具栏“保存文档”隐藏 /
 ******************************************************************************/
function bToolBar_HideSave() {
	try {
		var webObj = document.getElementById("WebOffice1");
		var vCurItem = webObj.HideMenuItem(0);
		// 根据vCurItem判断当前按钮是否显示
		webObj.HideMenuItem(0x04); // Hide it
	} catch (e) {
		alert("异常\r\nError:" + e + "\r\nError Code:" + e.number
				+ "\r\nError Des:" + e.description);
	}
...
/*******************************************************************************
 * 功能：在演示如何调用VBA接口 WebOffice提供GetDocumentObject()的接口导出对象 Word
 * 导出的是：MSWord::_Document Excel导出的是: MSExcel::_Workbook WPS 导出的是: WPS::_Document
 * 列子： 1.通过VBA获取当前用户的用户名
 * document.all.WebOffice1.GetDocumentObject().Application.UserName; 2.获取文档的标题
...
/*******************************************************************************
 * 
 * 显示或隐藏印章工具栏 通过录制VBA查看工具栏的名称。 然后可以采用下面方式来显示或隐藏 /
 ******************************************************************************/
...
/*******************************************************************************
 * Office2007菜单隐藏和恢复 ---全部菜单隐藏 /
 ******************************************************************************/
function allHideMenu_onclick() {
	try {
		var webObj = document.getElementById("WebOffice1");
		webObj.HideMenuAction(1, 0x100000 + 0x200000 + 0x400000 + 0x800000
...
/*******************************************************************************
 * Office2007菜单隐藏和恢复 ---恢复至设置之前状态（菜单显示，复制，粘贴可用） /
 ******************************************************************************/
...
function hideSeal() {
	...
	try {
		...
		if (obj != null) {
			obj.Application.CommandBars("电子印章").Visible = !obj
					.CommandBars("电子印章").Visible;
...
function ChangeFace(kind) {
	if (kind == 1) {
		...
		document.all.WebOffice1.SetCustomColour(2, 250 * 256 * 256 + 244 * 256
				+ 243);
		document.all.WebOffice1.SetCustomColour(1, 236 * 256 * 256 + 226 * 256
				+ 225);
		document.all.WebOffice1.SetCustomColour(3, 181 * 256 * 256 + 151 * 256
				+ 153);
		document.all.WebOffice1.SetCustomColour(0, 247 * 256 * 256 + 243 * 256
				+ 243);
	}
	if (kind == 2) {
		document.all.WebOffice1.SetCustomColour(2, 59 * 256 * 256 + 97 * 256
				+ 131);
		document.all.WebOffice1.SetCustomColour(1, 138 * 256 * 256 + 216 * 256
				+ 239);
		document.all.WebOffice1.SetCustomColour(3, 53 * 256 * 256 + 94 * 256
				+ 133);
		document.all.WebOffice1.SetCustomColour(0, 52 * 256 * 256 + 92 * 256
				+ 130);
	}
	if (kind == 3) {
		document.all.WebOffice1.SetCustomColour(2, 254 * 256 * 256 + 235 * 256
				+ 220);
		document.all.WebOffice1.SetCustomColour(1, 249 * 256 * 256 + 219 * 256
				+ 196);
		document.all.WebOffice1.SetCustomColour(3, 228 * 256 * 256 + 174 * 256
				+ 136);
		document.all.WebOffice1.SetCustomColour(0, 249 * 256 * 256 + 214 * 256
				+ 191);
...
function hideAll(pcExcludeBar1, pcExcludeBar2, pcExcludeBar3, pcExcludeBar4) {
	document.all.WebOffice1.HideMenuArea(pcExcludeBar1, pcExcludeBar2,
			pcExcludeBar3, pcExcludeBar4);
}
